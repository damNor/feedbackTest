...
const Component = () => {
    ...
    useEffect(()=>{ if(departments===undefined) navigate.push(`/${id}/`) },[departments])
    ...
    const onDepartmentSelect = async (dept) => {
        ...
        if(features && features.queue){
            ...
            toggle(false)
            if(qservices.error){ setError(qservices.error); return;}
            qservices = qservices[0].dept_service_data.map(i=>{ return{...i,...{enable_apmt:0}}})
        ...
        if(features&& features.appointment){
            ...
            toggle(false)
            if(aservices.error){setError(aservices.error); return;}

            aservices.map((item,i)=>{
                const apmt  = {...item,...{enable_apmt:1}}
                const isExist = finallist.find(j=>item.serv_id == j.serv_id)
                if(isExist) finallist = finallist.map(j=>item.serv_id == j.servid?{...apmt,...item}:j)   // if exist update the value
                else finallist.push(apmt)                                                                // else push it to the main
            })
        }
        console.log('services',finallist);
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
    ...
    <Content>
        ...
        { departments && departments.map((item,i)=><Department0 key={i} item={item}/>) }
    </Content>
    <Background/>
    <BottomBar/>
